...
if __name__ == '__main__':
    epochs = 50
    for idx,learningrate in enumerate(0.0001*2**np.arange(-1,2)):
        model_conv = get_model()
        #定义使用的损失函数为交叉熵代价函数
        criterion = nn.CrossEntropyLoss()
        #定义使用的优化器
        #optimizer_conv = optim.SGD(model_conv.parameters(),lr=learningrate,momentum=.9,weight_decay=1e-4)
        #optimizer_conv = optim.RMSprop(model_conv.parameters(),lr = 0.0001, momentum=.9,weight_decay=1e-4)
        optimizer_conv = optim.Adam(model_conv.parameters(), lr=learningrate, betas=(0.9, 0.99))
        ...
        model_train,train_acc,val_acc,train_loss,val_loss = train_model(model_conv, criterion, optimizer_conv, exp_lr_scheduler,num_epochs=epochs)
        plt.figure
        ...
        plt.xlabel('Number of epochs')
        plt.show()
        torch.save(model_train, 'GenderRES_adam_{}.pkl'.format(idx+1))